...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function isEmailChangedAndExists(userId, newEmail) {
    ...
    if (snapshot.exists()) {
      ...
      snapshot.forEach((childSnapshot) => {
        const user = childSnapshot.val();
        if (user.email.toLowerCase() === newEmail.toLowerCase() && childSnapshot.key !== userId) {
          emailExists = true;
        }
      });
      ...
      if (logsSnapshot.exists()) {
        logsSnapshot.forEach((childSnapshot) => {
          ...
          if (log.target_user_id === userId || 
              (log.message && log.message.toLowerCase().includes(user.email.toLowerCase()))) {
            userLogs.push({
              ...log,
  ...
  function showUserLogsModal(user, logs) {
    ...
    document.getElementById('logsUserEmail').textContent = user.email;
...
form.addEventListener("submit", (e) => {
  ...
  const newCashier = {
    ...
    password: document.getElementById("password").value, // hash in production
    role: document.getElementById("role").value,
    status: document.getElementById("status").value,
    update_at: new Date().toISOString()
  };

  // Call addCashier here
